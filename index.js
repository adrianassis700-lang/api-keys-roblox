const express = require('express');
const { createClient } = require('@supabase/supabase-js');
const app = express();

// Permite requisições no formato JSON
app.use(express.json());

// Habilita CORS para permitir conexões do Roblox e executores
app.use((req, res, next) => {
    res.header('Access-Control-Allow-Origin', '*');
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
    res.header('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
    if (req.method === 'OPTIONS') {
        return res.sendStatus(200);
    }
    next();
});

// Credenciais do Supabase
const supabaseUrl = 'https://yqxtabfebukvqexvaejs.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlxeHRhYmZlYnVrdnFleHZhZWpzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODU4NzIzOTksImV4cCI6MjEwMTQ0ODM5OX0.zFgeJkAkd3RHvwWnAgA_uiuYav02_0rtOhW7meQ6_F0';
const supabase = createClient(supabaseUrl, supabaseKey);

// Rota para testar se a API está ativa
app.get('/', (req, res) => {
    return res.json({ status: "API Online!", mensagem: "Servidor rodando perfeitamente." });
});

// Rota de Validação com Vínculo Vitalício por Usuário
app.post('/api/validar', async (req, res) => {
    try {
        const { key, userid } = req.body;

        if (!key || !userid) {
            return res.json({ valido: false, mensagem: "Dados incompletos." });
        }

        const chaveLimpa = String(key).trim();
        const idUsuario = String(userid);

        // Busca a key no banco de dados
        const { data, error } = await supabase
            .from('keys_sistema')
            .select('*')
            .ilike('chave', chaveLimpa)
            .single();

        if (error || !data) {
            return res.json({ valido: false, mensagem: "Key não encontrada." });
        }

        // Se a key já foi ativada anteriormente:
        if (data.usada) {
            if (data.user_id === idUsuario) {
                // É o mesmo usuário tentando entrar de novo!
                return res.json({ valido: true, mensagem: "Bem-vindo de volta!" });
            } else {
                // Outra pessoa tentando usar a chave de alguém
                return res.json({ valido: false, mensagem: "Esta key já pertence a outro usuário." });
            }
        }

        // Primeiro uso da key: vincula ao UserId atual
        await supabase
            .from('keys_sistema')
            .update({ usada: true, user_id: idUsuario })
            .eq('id', data.id);

        return res.json({ valido: true, mensagem: "Key vinculada com sucesso!" });

    } catch (err) {
        return res.status(500).json({ valido: false, mensagem: "Erro interno no servidor." });
    }
});

module.exports = app;

if (process.env.NODE_ENV !== 'production') {
    const PORT = process.env.PORT || 3000;
    app.listen(PORT, () => {
        console.log(`Servidor rodando na porta ${PORT}`);
    });
}
